...
export default function AttendanceBox({ session }: IAttendanceBox) {
  return (
    <div className="border-gray-800 bg-highlight flex rounded-xl overflow-hidden border cursor-pointer hover:border-highlight-200 group">
      ...
      <div className="p-4">
        ...
        <p className="text-gray-700 text-sm">Hiệu lực trong {session.duration} phút</p>
        <p className="text-gray-700 text-sm">Bắt đầu lúc {formatTime(session.start)}</p>
      </div>
    </div>
  );
}
